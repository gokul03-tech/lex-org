...
import { FileQuestion, Sparkles, Clock, Scale, Quote, FileText } from 'lucide-react';
...
export default function OverviewPage() {
  const { caseId, data, isLoading, refresh, openChat } = useWorkspace();
            ...
            <ol className="relative space-y-4 border-l border-slate-200 pl-6">
              {a.timeline.map((event, idx) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`absolute -left-[27px] top-1 flex h-3 w-3 items-center justify-center rounded-full ring-4 ring-white ${
                        isTail ? 'bg-gradient-to-tr from-sky-600 to-indigo-600' : 'bg-slate-300'
                      }`}
                    />
                    <div className="flex flex-wrap items-baseline gap-x-2.5">
                      ...
                      {event.page && (
                        <span className="inline-flex items-center gap-1 rounded border border-slate-200 bg-white px-1.5 py-0.5 font-mono text-[9px] text-slate-400">
                          <FileText className="h-2.5 w-2.5" /> p.{event.page}
                        </span>
                      )}
                    </div>
                    <p className="mt-1 max-w-3xl text-[13.5px] leading-relaxed text-slate-700">{event.fact}</p>
                  </motion.li>
                );
              })}
            </ol>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {issues.map((issue, idx) => (
              <motion.div
                ...
                whileHover={{ y: -2 }}
                className="space-y-3 rounded-2xl border border-slate-200/80 bg-white/80 p-5 shadow-sm backdrop-blur-md transition hover:shadow-md"
              >
                ...
                <p className="font-serif text-[15px] font-semibold leading-snug tracking-tight text-slate-900">
                ...
                {issue.evidence && (
                  <div className="rounded-xl border border-slate-100 bg-slate-50 p-3">
                    ...
                    {issue.page && (
                      <span className="mt-1.5 inline-block rounded border border-slate-200 bg-white px-1.5 py-0.5 font-mono text-[9px] text-slate-400">
                        Page {issue.page}
                      </span>
                    )}
                  </div>
