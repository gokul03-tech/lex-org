...
_FIRST = ["Aarav","Vivaan","Aditya","Vihaan","Arjun","Sai","Reyansh","Krishna","Ishaan","Rudra","Om","Kabir","Advait","Dhruv","Aryan","Rohan","Karan","Nikhil","Amit","Sunil","Anil","Vijay","Sanjay","Ajay","Ravi","Deepak","Pankaj","Ashok","Varun","Tarun","Rakesh","Mukesh","Suresh","Mahesh","Dinesh","Lokesh","Naresh","Umesh","Kamesh","Ritesh","Ananya","Diya","Aadhya","Myra","Sara","Ira","Pihu","Anika","Navya","Kiara","Riya","Simran","Priya","Neha","Pooja","Swati","Divya","Meera","Kavya","Shreya","Nisha","Trisha","Meghna","Payal","Charu","Bhavna","Hetal","Jigna"]
_LAST = ["Sharma","Verma","Gupta","Mehta","Patel","Shah","Desai","Joshi","Kulkarni","Deshpande","Iyer","Menon","Nair","Pillai","Reddy","Rao","Naidu","Murthy","Banerjee","Chatterjee","Mukherjee","Das","Sen","Bose","Ghosh","Singh","Yadav","Tiwari","Mishra","Pandey","Shukla","Dubey","Malhotra","Kapoor","Chopra","Khanna","Arora","Bhatia","Kaur","Sandhu","Sheikh","Khan","Ansari","Qureshi","Pasha","Setty"," Gowda","Naik","Pai"," Kamath"]
PERSON_NAMES = [f"{f} {l}" for f in _FIRST[:40] for l in [x.strip() for x in _LAST[:14]]][:560]
...
def main():
    import sys
    start_num = int(sys.argv[1]) if len(sys.argv) > 1 else 6
    end_num = int(sys.argv[2]) if len(sys.argv) > 2 else 50
    global random
    random = random.Random(1000 + start_num)  # deterministic per batch
    ...
    while n <= end_num:
        ...
        seq_in_cat[cat] += 1
        title, header, body = builder(seq_in_cat[cat] * 13 + n)
        first_party = title.split(" vs ")[0].strip().replace(" ", "_").replace(".", "")
        filename = f"{n:03d}_{label}_{first_party[:28]}.pdf"
        if filename not in have:
            ...
            print(f"Generated {filename} ({pages} pages)", flush=True)
        n += 1
        cat = (cat + 1) % len(BUILDERS)

    with open(manifest_path, "w") as fh:
        json.dump(manifest, fh, indent=2)
    print(f"Done. Total manifest entries: {len(manifest)}")
